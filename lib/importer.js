...
const importFile = async ({ filePath, stateData, repo, ref, force }) => {
  // set here
    ...
    specs.servers =
      ...
      specs.servers.length > 0
        ...
        : [
            ...
            {
              url: `${configValues.ingressUrl}/${currentState.organization}/${currentState.serviceId}`,
...
module.exports.importWithOptions = async function main(args) {
  ...
  if (
    ...
    !repo
      .toLowerCase()
      .startsWith(configValues.githubOrganization.toLowerCase() + "/")
  ) {
    console.error(
      "Expected repository owner is:",
      configValues.githubOrganization,
      "Got: ",
      path.dirname(repo)
    );
    process.exit(1);
      ...
      try {
        ...
        if (e.response && e.response.status === 404) {
          ...
          if (idx >= 0) {
            ...
            removeFile(removedSpec.targetPath);
            removed++;
          }
        } else {
          throw e;
