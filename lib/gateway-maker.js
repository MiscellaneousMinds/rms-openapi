...
const GATEWAY_API_VERSION = configValues.version;
...
const getBaseGateway = (organization, overrides = {}) => {
  const baseGateway = {
    ...
    info: {
      ...
      title: `${_.upperFirst(organization)} API Gateway`,
      description: `**Introduction**
API Gateway for ${_.upperFirst(organization)}
...
module.exports.gatewayBuilder = async ({ organization }) => {
  if (!/^[a-z0-9-]+$/gi.test(organization)) {
    throw new Error("organization MUST be alpha-numeric");
  }

  const targetDir = path.join(SPECS_DIR, organization);
  ...
  if (!ORGANIZATIONS_WITH_NO_GATEWAY.includes(organization)) {
    const gatewaySpec = await resolveRefs(
      buildGatewaySpecs(organization, orgSpecs)
    );
    persistSpecs("api-gateway", targetDir, gatewaySpec);
  } else {
    console.log("Skipping API gateway generation for:", organization);
  }

  if (ORGANIZATIONS_WITH_EXTERNAL_APIS.includes(organization)) {
    const gatewayOverride = {
      [SERVICE_ID]: "external-gateway",
      info: {
        title: `${_.upperFirst(organization)} External API Gateway`,
      },
    };
...
module.exports.makeFromStates = async (states) => {
  ...
  for (const [organization] of allOrgs.entries()) {
    console.log("Building API gateway for:", organization);
    await this.gatewayBuilder({ organization });
  }
};
